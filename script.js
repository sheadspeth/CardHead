...
async function loadContracts() {
  try {
    ...
    if (!companyData.networks[networkId] || !marketplaceData.networks[networkId] || !objectCardData.networks[networkId] || !objectCardHeadData.networks[networkId] || !reviewData.networks[networkId]) {
      throw new Error("One or more contracts not deployed on the current network. Check your migrations and network IDs.");
    }
    ...
    console.log("✅ Contracts loaded:", {
      company: company.options.address,
      marketplace: marketplace.options.address,
      objectCard: objectCard.options.address,
      objectCardHead: objectCardHead.options.address,
      review: review.options.address
    });
...
async function uploadCard() {
    ...
    const companyAddr = document.getElementById('companyDropdown').value;
    const series = document.getElementById('seriesDropdown').value;
    ...
    if (!name || !companyAddr || !series || !edition || !file) {
        alert("Please fill all fields");
        return;
    }

    const companyName = await company.methods.getCompanyName(companyAddr).call();
    const ipfsUrl = await uploadToIPFS(file);
    const nextBarcode = await objectCard.methods.getNextBarcode().call();

    await objectCard.methods.addCard(
        name,
        companyName, // game = company name (Pokemon, etc)
        series,
        grade,
        edition,
        0, // Default rating 0
    ...
    await objectCardHead.methods.addCardToUser(nextBarcode).send({ from: accounts[0] });

    alert("Card uploaded and assigned!");
}
...
//html functions
    ...
    async function populateCompanyDropdown() {
      ...
      //hardcode for demo
      const knownCompanyAddresses = [
          "0xCompanyAddress...",
      ];
  
      for (let address of knownCompanyAddresses) {
          const companyName = await company.methods.getCompanyName(address).call();
  ...
  document.getElementById('companyDropdown').addEventListener('change', async function() {
      const companyAddress = this.value;
      const seriesDropdown = document.getElementById('seriesDropdown');
  
      // Clear previous series options
      seriesDropdown.innerHTML = '<option value="">Select Series</option>';
  
      if (companyAddress) {
          const seriesList = await company.methods.getCompanySeries(companyAddress).call();
  ...
  async function createCompany() {
    ...
    try {
        await addCompany(companyName);  // Calling the blockchain function
...
async function addSeriesToCompany() {
    const seriesName = document.getElementById('newSeriesName').value;
    if (!seriesName) {
        alert("Please enter a series name");
        return;
    }
    await addSeries(seriesName); // your already existing function
}

async function viewCompanyInfo() {
    const companyName = await getCompanyName(accounts[0]);
    const seriesList = await getCompanySeries(accounts[0]);
...
window.addEventListener("load", async () => {
    if (window.ethereum) {
       ...
       /* marketplace = new web3.eth.Contract(marketplaceABI, marketplaceAddress);
        ...
        if (addSeriesBtn) addSeriesBtn.addEventListener('click', addSeriesToCompany);

        const viewCompanyBtn = document.getElementById('viewCompanyBtn');
        if (viewCompanyBtn) viewCompanyBtn.addEventListener('click', viewCompanyInfo);
